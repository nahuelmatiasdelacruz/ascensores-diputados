...
export default function TabsContainer({setData,data,handleCloseEditar}) {
  ...
  const confirmarCambios = async () => {
    try{
      const result = await axios.put(`${server}/api/empleados/`,userData);
  ...
  const handleSync = async () => {
    setLoading(true);
    try{
        await axios.get(`${server}/api/huellas/sync/${userData.empleado_id}`);
        setLoading(false);
        toast.success("Sincronización de huellas en equipos COMPLETADA");
    }catch(e){
        setLoading(false);
        toast.error(`No se pudo sincronizar los dispositivos: \n${e.message}`);
    }
}
  ...
  const updateUserData = async () => {
    try{
      ...
      const empleado = await axios.get(`${server}/api/empleados/${data.empleado_id}`);
      const newData = {
        ...empleado.data,
  ...
  return (
    <Box sx={{ width: '90%', backgroundColor: "white", margin: "0 auto", marginTop: "30px", boxSizing: "border-box", padding: "20px",borderRadius: "6px"}}>
      <Toaster/>
      ...
      <div className="profile-data-header">
        <div className='profile-photo-data'>
          <div className="profile-photo-container">
            <img src={data.profilePhoto !== null ? `data:image/png;base64,${data.profilePhoto}` : UserPhoto} alt="profile photo"/>
          </div>
          <div className="button-upload-container">
              ...
              >
                ...
                <MenuItem onClick={startWebcamSync}>WebCam</MenuItem>
              </Menu>
          </div>
        </div>
        <div className="profile-data">
          ...
          <Stack direction="row" spacing={1}>
            <Chip label={userData.estado} color={getColor(userData.estado)} size="small"/>
          </Stack>
        </div>
      </div>
      ...
      <TabPanel value={value} index={1}>
        <HabilitacionesTab updateData={updateUserData} userData={userData}/>
        <Stack direction="row" spacing={5} marginTop="30px" justifyContent="center">
          <Button onClick={handleCloseEditar} color="error" variant="outlined">Salir</Button>
          <Button onClick={handleSync} disabled={loading} variant="contained" startIcon={<SyncIcon/>}>Sincronizar</Button>
        </Stack>
      </TabPanel>
